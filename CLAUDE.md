...
3. **Multi-size (6×6 / 9×9 / 16×16)** — **6×6 + 9×9 SHIPPED (all 3 layers); 16×16 engine-ready, not yet exposed.** Every variant (classic/X/jigsaw/killer) is now playable at 6×6 and 9×9. The 16×16 solver prerequisite (constraint propagation) is DONE and 16×16 generate+grade is now sub-second per-request; only server/web enablement remains. Spec/plans in `docs/superpowers/{specs,plans}/2026-05-31-*` and `2026-06-01-solver-constraint-propagation*`.
   ...
   - **16×16 is NOW per-request FEASIBLE** (measured, not yet exposed). With propagation: 16×16 classic **generate ~35 ms + grade ~20 ms ≈ sub-second** at the 47% clue floor (was the "~14 s / grading-minutes" blocker — that was the *minimal-clue* regime). Uniqueness checks stay ~1 ms to ~110 clues, 137 ms at 100, then explode, so the **n>9 floor is kept at 47%** (robust + sub-second; lowering buys marginally harder puzzles at real variance cost). Engine bench: `bench_16x16_uniqueness_and_generation` (`#[ignore]`). **Shipping 16×16 now (Classic + X-Sudoku only):** server `size=16` acceptance (currently rejected) + web `Size` `6|9`→`6|9|16` + A–G digit codec/notes-width widening + selector + per-size tier set. Design: `docs/superpowers/specs/2026-06-02-16x16-ship-design.md`. Rides out with the propagation branch as one deploy. The Postgres pool (#5) is NOT required for the generate+grade path.
   - **Deferred 16×16 follow-up — Jigsaw + Killer at 16×16:** not in the first 16×16 ship. **Jigsaw@16** is blocked on the pre-existing `fill_random` perf tail (below) — irregular-box generation is already 40s+ at 9×9 and worse at 256 cells; fix `fill_random` (MRV / reuse `SolveCtx`) first. **Killer@16** needs cage-generation viability at 256 cells plus cage rendering/entry UX on a 16×16 grid (and grading cost under the cage gate). Both want the variant×size timing benchmarked before exposure. Sequence: land Classic+X 16×16 → fix jigsaw `fill_random` → add Jigsaw@16 → scope Killer@16.
   - **Pre-existing perf tail (flagged, separate task):** 9×9 *jigsaw* generation can take 40 s+ on unlucky seeds — `generator.rs` `fill_random` uses naive first-empty backtracking (not the new solver) and chokes on irregular boxes. Fix: give `fill_random` MRV ordering or reuse `SolveCtx`. Affects `/api/puzzle?variant=jigsaw`. Not introduced by the propagation work.
