...
12. **Future-proof the "free / no ads / no signup" copy** — small, do before any accounts/ads land. The variant FAQs + `llms.txt` currently promise "no signup, no ads, no in-app purchases" (absolute). If accounts (for #11 leaderboards) or ads are ever likely, soften *now* to e.g. "free to play — **no signup required to start a puzzle**" and drop/hedge the absolute "no ads," because retracting "no ads, ever" damages trust more than never promising it. Edit spans the 4 landing pages + `web/public/llms.txt` (and the FAQ JSON-LD must keep matching the visible copy).
